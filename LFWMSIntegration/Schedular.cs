...
namespace LFWMSIntegration
{
    class Schedular
    {
        ...
        double interval = Convert.ToDouble(ConfigurationManager.AppSettings["Interval"]) * 60000;
        StringBuilder sbLog;
        ...
        void oTimer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            try
            {
                ...
                foreach (string comp in lstCompanies)
                {
                    ...
                    int P101rows = oDAL.GetRows(LuckyConStr, P101procName);
                    if (P101rows > 0)
                    {
                        if (!CSVFileExist(P101fileName))
                            SendDataToCSVFile(LuckyConStr, P101fileName, P101procName, PartNumcolumn);
                    }
                    ...
                    int P103rows = oDAL.GetRows(LuckyConStr, P103procName);
                    if (P103rows > 0)
                    {
                        if (!CSVFileExist(P103fileName))
                            SendDataToCSVFile(LuckyConStr, P103fileName, P103procName, uomColumn);
                    }
                    */
        ...
        private void SendDataToCSVFile(string dbConStr,string fileName,string storeProcName,string colName)
        {
                            ...
                            if (keyColToUpd != string.Empty)
                            {
                                    ...
                                    switch (colName)
                                    {
                                        ...
                                        case "Calculated_PartNum":
                                            ...
                                            oDAL.UpdatePartCheckBox14(keyColToUpd, dbConStr);
                                            break;
                                        case "Part_PartNum":
                                            //Part_UD.CheckBox15 needs to update
                                            oDAL.UpdatePartCheckBox15(keyColToUpd, dbConStr);
                                            break;
                                        */
                                        //Part is separated in another program - End
